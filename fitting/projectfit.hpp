...
namespace project
{
  ...
  namespace fit
  {
    ...
    std::map<double_t, std::complex<double_t>> analyse_periodicity(std::map<K, V> t_series_data, std::size_t block_size = 1)
    {
      ...
      V avg = std::accumulate(t_series_data.begin(), t_series_data.end(), 0, [](auto a, auto b)
                              { return a + b.second; }) /
              (V)t_series_data.size();
      V var = std::accumulate(t_series_data.begin(), t_series_data.end(), 0, [avg](auto a, auto b)
                              { return a + (b.second - avg) * (b.second - avg); }) /
              (V)(t_series_data.size() - 1);
